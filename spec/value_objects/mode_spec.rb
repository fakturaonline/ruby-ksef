...
RSpec.describe KSEF::ValueObjects::Mode do
  ...
  describe "#default_url" do
    it "returns test URL for test mode" do
      mode = described_class.new(:test)
      expect(mode.default_url).to eq "https://api-test.ksef.mf.gov.pl/v2"
    end

    it "returns demo URL for demo mode" do
      mode = described_class.new(:demo)
      expect(mode.default_url).to eq "https://api-demo.ksef.mf.gov.pl/v2"
    end

    it "returns production URL for production mode" do
      mode = described_class.new(:production)
      expect(mode.default_url).to eq "https://api.ksef.mf.gov.pl/v2"
    end
  end
end
